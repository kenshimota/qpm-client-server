<template>
  <div class="white" style="padding: 20px; border: 1px solid #ddd">
    ...
    <v-row>
      <v-col cols="12" md="6">
        <v-row  style = "border: 1px solid #ddd; border-top: 3px solid #ddd; border-radius: 5px; padding: 10px;" >
          <v-col cols="12">
            <v-autocomplete
              v-model="method"
              :items="methodsApi"
          ...
          <v-row v-if="!method == false && !loadingInput">
            <v-col
              v-for="(input, key) in configTestApi[method].requires"
              cols="12"
              md="6"
              :key="input + '-' + key"
            >
            ...
            <v-col cols="12">
              <v-btn color = "grey darken-2" dark @click = "getRequestApi">
...
export default {
  ...
  created: function () {
    ...
    for (var index in this.configTestApi) {
      methods.push(index);
    }
  ...
  methods: {
    ...
    getRequestApi: async function(){
      try {
        ...
        let response = await Client.method(this.method, data).fetch().then((t) => t);
